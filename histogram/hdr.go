...
func New(minValue, maxValue int64, significantFigures int) *Histogram {
	...
	smallestUntraceableValue := int64(subBucketCount) << uint(unitMagnitude)
	bucketsNeeded := int32(1)
	for smallestUntraceableValue < maxValue {
		smallestUntraceableValue <<= 1
		bucketsNeeded++
	}
...
// Bar Histogram bar for plotting
type Bar struct {
	From, To, Count int64
}
